...
async function handleFollowUpQuestion(
  ...
  if (followUpType === "question_detail" || followUpType === "reference_previous") {
    ...
    // "semuanya yg dia punya" / "punya dia" — show all tasks for last mentioned member
    if (/semuanya|semua|punya\s+dia|yg\s+dia|yang\s+dia/i.test(lower)) {
      if (session.lastMemberName || session.activeMemberName) {
        const memberName = session.lastMemberName || session.activeMemberName!;
        try {
          const items = await getBacklogByMemberName(memberName);
          if (items.length === 0) {
            return `Wah, saat ini belum ada tugas yang di-assign ke *${memberName.split(" ")[0]}* nih.`;
          }
          let msg = `*Tugas ${memberName.split(" ")[0]}* (${items.length} item)\n\n`;
  ...
  if (extractedMemberName) {
    ...
    try {
      ...
      if (isNotifyIntent && !wantsOwnTasksOnly) {
        ...
        console.log(`[Agent] Notify ALL backlog to "${displayName}" from "${message.slice(0, 60)}"`);
        const allItems = await listBacklog(undefined, [
          { property: "Status", direction: "ascending" },
        ]);

        if (allItems.length === 0) {
          return await addCasualTouch(`Wah, backlog SGA kosong nih. Gak ada yang perlu aku kirim ke ${displayName}.`, context);
        }
        ...
        let msg = `*Master Backlog* (${allItems.length} item)\n\n`;
        ...
        // Send WA notification to the member with ALL backlog
        try {
          ...
          if (picContact) {
            ...
            const notifyMsg = `Hai ${displayName}! \u{1F44B}\n\n${senderName} minta aku buat ngasih info *SELURUH* backlog SGA:\n\n` +
              Object.entries(grouped).map(([status, group]) =>
                ...
                group.slice(0, 5).map((item, i) => `  ${i + 1}. ${item.name} [${item.priority}]`).join("\n") +
                (group.length > 5 ? `\n  ...dan ${group.length - 5} lainnya` : "")
              ).join("\n\n") +
              `\n\nTotal: ${allItems.length} item. Cek detailnya di Notion ya!`;
            await sendDirectMessage(env.EVOLUTION_INSTANCE_NAME, picContact.phone, notifyMsg);
            msg += `Pesan sudah aku kirim ke ${displayName} via WA! \u2705`;
            console.log(`[Agent] ALL backlog notification sent to ${displayName} (${picContact.phone})`);
          } else {
        ...
        // Save context
        session.lastMemberName = searchName;
        session.activeMemberName = searchName;
        session.lastNotionResults = allItems.map(item => ({
          name: item.name,
          id: item.id,
          url: item.url,
          status: item.status,
      ...
      // Default: show member's own tasks (when explicitly asked: "tugas ivan", "task punya ivan")
      ...
      // Save member context for follow-up questions
      session.lastMemberName = searchName;
      session.activeMemberName = searchName;
      session.lastNotionResults = items.map(item => ({
        name: item.name,
        id: item.id,
        url: item.url,
        status: item.status,
