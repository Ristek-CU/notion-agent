...
// ─── Manual LID Map (from env LID_PHONE_MAP) ─────────────────────────
// Format: "lidJid1:phone1,lidJid2:phone2"
const manualLidMap = new Map<string, string>();
function loadManualLidMap() {
  const raw = env.LID_PHONE_MAP;
  if (!raw) return;
  for (const entry of raw.split(",")) {
    const [lid, phone] = entry.split(":").map(s => s.trim());
    if (lid && phone) {
      manualLidMap.set(lid, phone);
      // Also store full JID form
      if (!lid.includes("@lid")) {
        manualLidMap.set(`${lid}@lid`, phone);
      }
    }
  }
  if (manualLidMap.size > 0) {
    console.log(`[WA Sender] Loaded ${manualLidMap.size} manual LID mappings`);
  }
}
loadManualLidMap();
...
async function resolveLidToPhone(lidJid: string, pushName?: string): Promise<string> {
  // Check manual LID map first (from LID_PHONE_MAP env)
  const manualNumber = manualLidMap.get(lidJid);
  if (manualNumber) {
    console.log(`[WA Sender] Resolved LID ${lidJid} → ${manualNumber} (via manual LID_PHONE_MAP)`);
    lidResolutionCache.set(lidJid, manualNumber);
    saveLidCache();
    return manualNumber;
  }
  ...
  try {
    ...
    // Strategy 7: Last resort — could not resolve
    console.warn(`[WA Sender] Could not resolve LID ${lidJid} to real phone number (pushName: ${effectivePushName || "unknown"})`);
    console.warn(`[WA Sender] Tip: Add manual mapping in .env: LID_PHONE_MAP=${lidJid}:<phone_number>`);
    return lidJid;
  } catch (error) {
